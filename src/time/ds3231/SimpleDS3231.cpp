...
{
    i2c_init(SENSIFY_DS3231_I2C, 400000);
    gpio_set_function(SENSIFY_DS3231_SDA, GPIO_FUNC_I2C);
    gpio_set_function(SENSIFY_DS3231_SCL, GPIO_FUNC_I2C);
    gpio_pull_up(SENSIFY_DS3231_SDA);
    gpio_pull_up(SENSIFY_DS3231_SCL);
}

void SimpleDS3231::_read_data_reg(uint8_t reg, uint8_t n_regs)
{
    _data_buffer[0] = reg;
    i2c_write_blocking(SENSIFY_DS3231_I2C, DS3231_ADDR, _data_buffer, 1, true);
    i2c_read_blocking(SENSIFY_DS3231_I2C, DS3231_ADDR, _data_buffer + reg + 1, n_regs, false);
}
...
void SimpleDS3231::_write_data_reg(uint8_t reg, uint8_t n_regs)
{
    ...
    i2c_write_blocking(SENSIFY_DS3231_I2C, DS3231_ADDR, buffer, n_regs + 1, false);
}
...
uint8_t SimpleDS3231::get_temp()
{
    ...
    i2c_write_blocking(SENSIFY_DS3231_I2C, DS3231_ADDR, _data_buffer, 1, true);
    i2c_read_blocking(SENSIFY_DS3231_I2C, DS3231_ADDR, &temp, 1, false);
